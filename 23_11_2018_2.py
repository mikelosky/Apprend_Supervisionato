...
class L_R:
    ...
    def l_r(self, candy_x, candy_y):
        ...
        for i in range(0, iteration):
            ...
            cost_history[i] = self.cost_function(theta, candy_1, candy_y)

        return theta, theta_history, cost_history

    def cost_function(self, theta, x, y):
        cost = np.sum(y * np.log((np.dot(x, theta)).T) + (1 - y) * (np.log(1 - np.dot(x, theta)))).T * (1 / len(y))
        return cost
...
if __name__ == '__main__':
    candy = pd.read_csv("candy_1.csv",
                        names=["caramel", "peanutyalmondy", "nougat", "crispedricewafer", "hard", "bar", "pluribus",
